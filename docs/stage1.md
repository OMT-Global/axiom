...
- Stage1 now ships a synthetic standard library surface under the `std/` import prefix with fifteen landed modules. Capability-gated modules include `std/time.ax`, `std/env.ax`, `std/fs.ax`, `std/fs_write.ax`, `std/net.ax`, `std/process.ax`, `std/crypto_hash.ax`, and `std/http.ax`. `std/fs.ax` exposes `read_file(path: string): Option<string>` on top of `fs_read`; `std/fs_write.ax` exposes write-side helpers `create_file`, `write_file`, `append_file`, `mkdir`, `mkdir_all`, `remove_file`, `remove_dir`, and `replace_file`. Read access requires `fs = true`; write-side helpers require the separate `fs_write = true` manifest key and are reported by `axiomc caps` as `fs:write`. Each of the other gated modules requires the importing package to declare the matching capability (`clock`, `env`, `net`, `process`, or `crypto`); environment access is scoped with `env = ["PORT", "LOG_LEVEL"]`, and `env_get` returns `None` for names outside that manifest allowlist. The legacy `env = true` form remains temporarily available but emits a check warning because it grants unrestricted process environment access; `env_unrestricted = true` is the explicit migration escape hatch and is reported as unsafe in capability output. Ungated modules include `std/io.ax`, `std/json.ax`, `std/collections.ax`, `std/string_builder.ax`, and `std/log.ax`, with additional deterministic runtime surfaces in `std/sync.ax` and `std/async.ax`.
