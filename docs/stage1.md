...
- Scalar `static NAME: T = expr` declarations are accepted at module scope, can be `pub` / `pub(pkg)` and imported with the same visibility rules as `const`, and are emitted as Rust `static` items for `int`, `bool`, and string-literal `string` initializers. Broader const/static work remains open: no `const fn`, no array-size constants, no address-taking syntax, and no non-scalar static values yet.
