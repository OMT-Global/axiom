...
- The backend-selection surface is only preparatory seam work for later native-backend expansion; today `generated-rust` is the only implemented backend, so this branch is part of #105 rather than closure for it.
