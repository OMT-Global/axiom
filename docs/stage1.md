...
cargo run --manifest-path stage1/Cargo.toml -p axiomc -- dap
```
...
`axiomc dap` serves a bounded Debug Adapter Protocol endpoint over stdio. It is
the current stage1 debugger integration point for editor clients and harnesses:

- `initialize` returns the versioned `axiom.stage1.dap.v1` capability payload.
- `launch` requires `arguments.program` and accepts optional `arguments.package`;
  it runs a debug build, reports the native binary and `debugMap`, and emits
  `initialized`.
- `setBreakpoints` validates requested `.ax` source lines against the
  `axiom.stage1.debug_map.v1` sidecar generated by `launch`, marking only mapped
  source lines as verified.
- `loadedSources`, `threads`, `stackTrace`, `scopes`, `variables`,
  `configurationDone`, `terminate`, and `disconnect` return stable bootstrap
  responses so DAP clients can complete setup without pretending local-variable
  inspection or stepping is implemented.

This is intentionally not a full interactive debugger yet. Native Axiom stepping,
locals, and richer stack inspection remain blocked on the direct backend / DWARF
debug-info track, but editor integrations can now validate DAP requests against
real compiled stage1 metadata instead of a placeholder.
