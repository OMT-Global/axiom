...
`axiom.toml`, including `kind = "unit"`, `"table"`, `"property"`, `"snapshot"`,
or `"benchmark"` for JSON reporting. Discovery classifies `*_table_test.ax`,
`*_property.ax`, `*_property_test.ax`, `*_snapshot_test.ax`, and
`*_golden_test.ax` as richer fixture kinds while preserving the ordinary
...
`skipped` counts. Workspace-only roots are supported as long as build/run
