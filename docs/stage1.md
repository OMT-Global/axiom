...
- The AG4.1 stdlib surface now covers every stage1 capability-gated intrinsic with a thin wrapper module (`std/time.ax`, `std/env.ax`, `std/fs.ax`, `std/net.ax`, `std/process.ax`, `std/crypto_hash.ax`) plus the first ungated stdlib module `std/io.ax` (`eprintln` on top of the new `io_eprintln` intrinsic). The remaining AG4.1 modules (`std.json`, `std.http`, `std.collections`, `std.sync`) require new stdlib intrinsics, the AG4.2 async runtime, or AG2 generics and stay as follow-on work.
...
- `make stage1-test stage1-smoke` now covers all nineteen checked-in stage1 examples.
