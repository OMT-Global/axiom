...
- `stage1/examples/stdlib_regex` extends AG4.1 with `import "std/regex.ax"`, bringing an ungated regex floor into scope: `is_match`, `find`, and `replace_all` support literals, escapes, `.`, anchors, `*`, `+`, `?`, and character classes through a bounded generated-runtime matcher.
