...
- `stage1/examples/stdlib_http` extends AG4.1/AG4.3 with `import "std/http.ax"`, bringing `get(url)` and `serve_once(bind, body)` into scope on top of blocking HTTP client/server helpers. It shares the importing package's `[capabilities] net` flag with `std/net.ax`; the checked-in example keeps its smoke deterministic by exercising the closed-port client path, while the Rust integration suite covers the single-request server path.
