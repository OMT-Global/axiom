...
- Auth:
  ...
  - fallback: add a repository secret named `ANTHROPIC_API_KEY`
    ...
    - Keep the Claude workflow out of the required PR check set. The required checks are `CI Gate`, `test (python 3.11 hosted)`, `test (python 3.12 self-hosted)`, `test (python 3.13 hosted)`.
