...
| [#927](https://github.com/OMT-Global/axiom/issues/927) Rust exit native backend parity readiness matrix | Implemented as the initial readiness gate | Close as completed when this PR lands | `docs/rust-exit-readiness.md` and `docs/rust-exit-readiness.json` define the blocked backend/bootstrap matrix, while `scripts/ci/check-rust-exit-readiness.sh` and `make rust-exit-readiness` emit `axiom.rust_exit.readiness.v1` JSON. The gate is expected to fail until the live blocker set in `docs/rust-exit-readiness.json` is closed: #1124 for direct-native runtime ABI coverage and #731 for LSP bootstrap parity. Final bootstrap issue #721 remains metadata and should not be listed as a live blocker for its own closing PR. |
