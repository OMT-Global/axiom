...
- `docs/stage1.md` is the short status page for the supported Rust-hosted
  `axiomc` workflow.
...
- Rust-hosted `axiomc` behavior under `stage1/`.
...
| [#927](https://github.com/OMT-Global/axiom/issues/927) Rust exit native backend parity readiness matrix | Implemented as the initial readiness gate | Close as completed when this PR lands | `docs/rust-exit-readiness.md` and `docs/rust-exit-readiness.json` define the readiness matrix and live open-blocker manifest, while `scripts/ci/check-rust-exit-readiness.sh` and `make rust-exit-readiness` emit `axiom.rust_exit.readiness.v1` JSON. The gate is expected to fail while the live blockers remain: #1124 for direct-native ABI completion, #731 for the AxiOM-owned LSP path, and #721 for final Rust-bootstrap removal. Historical generated-Rust/default-backend issues #693 and #694 are closed and are no longer the gating tracker for removal proof. |
