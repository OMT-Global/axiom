...
| Direct native runtime ABI | Supported values, ownership shapes, stdlib calls, and capability host calls lower through backend-neutral direct-native runtime entrypoints. | Partial and checked by `scripts/ci/check-direct-native-runtime-abi.py`; owned move-state and capability shims are implemented, while the remaining value-feature rows still block through #1001. | [#1001](https://github.com/OMT-Global/axiomlang/issues/1001) |
