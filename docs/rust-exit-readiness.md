...
| Default backend | `axiomc build`, `axiomc run`, and `axiomc test` default to direct native output and no longer invoke `rustc` for supported broad native workflows. | Host/native build, run, and test commands default to the direct-native Cranelift backend; targeted builds and the generated-Rust backend alias still keep generated Rust until #1191 is complete. | [#1191](https://github.com/OMT-Global/axiomlang/issues/1191) |
