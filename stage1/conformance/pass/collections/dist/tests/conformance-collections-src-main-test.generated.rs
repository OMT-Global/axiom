...
const AXIOM_PACKAGE_ROOT: &str = "/Users/johnteneyckjr./src/axiom/stage1/conformance/pass/collections";
const AXIOM_FS_ROOT: &str = "/Users/johnteneyckjr./src/axiom/stage1/conformance/pass/collections";
...
fn conformance_collections_main_test_second<'a>(values: &'a [i64]) -> i64 {
    return axiom_array_get(values, 1);
}

fn main() -> std::process::ExitCode {
    axiom_install_panic_hook();
    let result = panic::catch_unwind(|| {
        let numbers: Vec<i64> = vec![4, 5, 6, 7];
        let window: &[i64] = axiom_slice_view(&numbers, Some(1), Some(3));
        let scores: HashMap<String, i64> = HashMap::from([(String::from("build"), 7), (String::from("deploy"), 9)]);
        let pair: (i64, String) = (11, String::from("tuple"));
        println!("{}", conformance_collections_main_test_second(axiom_slice_view(&numbers, None, None)));
        ...
        println!("{}", (window).len() as i64);
        println!("{}", axiom_map_get(&scores, &String::from("deploy")));
        println!("{}", (pair).0);
        println!("{}", (pair).1);
    });
    match result {
