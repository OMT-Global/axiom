...
"#;
    ...
    fn stage1_project_imports_synthetic_stdlib_regex_module() {
        // `std/regex.ax` is ungated in stage1: it provides deterministic
        // string matching in the generated runtime without host I/O access.
        let dir = tempdir().expect("tempdir");
        ...
        .expect("write lockfile");
        let source = "import \"std/regex.ax\"\nprint is_match(\"^[a-z]+[0-9]+$\", \"agent42\")\nprint is_match(\"^[a-z]+[0-9]+$\", \"agent\")\nmatch find(\"[0-9]+\", \"agent42-build7\") {\nSome(value) {\nprint value\n}\nNone {\nprint \"missing\"\n}\n}\nprint replace_all(\"[0-9]+\", \"agent42-build7\", \"#\")\nprint is_match(\"colou?r\", \"color\")\nprint is_match(\"colou?r\", \"colour\")\nprint is_match(\"a.*z\", \"abcz\")\n";
        fs::write(project.join("src/main.ax"), source).expect("write source");
        fs::write(project.join("src/main_test.ax"), source).expect("write test");
        fs::write(
            project.join("src/main_test.stdout"),
